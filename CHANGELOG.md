...
- Rework date range picker UX in CALENDAR view: collapse the two OD/DO inputs into a single read-only input that displays `MMM dd, y - MMM dd, y` with a leading calendar icon, and open the calendar as an absolutely-positioned popover anchored to the input width (click-outside via `useOnClickOutside`) instead of pushing page content. `DateRangeWrapper` props simplified — `startDatePlaceholderText` + `endDatePlaceholderText` collapsed to a single `placeholder`, and seven other unused props (`hideKeyboardShortcutsPanel`, `monthFormat`, `displayFormat`, `weekDayFormat`, `verticalSpacing`, `daySize`, `transitionDuration`, `regular`) removed. Only consumer (`ServiceDateRange`) updated accordingly.
- Restyle the calendar popover to align with shadcn/AirBnB conventions: target the v9 `.rdp-root` selector (the prior `.rdp` overrides were dead — v9 emits `rdp-root` per `getDefaultClassNames.js`), set `--rdp-accent-color` to `theme.colors.primary` (no more default blue), `--rdp-accent-background-color` to `colorSchemas.background.secondary.color` for the range-middle pill, force `flex-wrap: nowrap` + `justify-content: space-evenly` on `.rdp-months` so two months stay side-by-side and distribute across the popover width, and pass `navLayout="around"` so prev/next chevrons sit on the outer edges of the months row. Selected dates use v9's native filled-circle range_start/range_end with the connected gradient pill spanning rows; `.rdp-selected { font-size: inherit }` keeps day numbers from upsizing on selection.
- `ServiceDateRange` card now uses `Card`'s default 20px padding (matches the `Leave your details` card) and forces `width: 100%` on the wrapper + the inner `Column`. `TimeZoneSelect` aligns to the right edge of the card content area (removed `$mr={1}` and `$pr={2}` that were pushing it inward by 24px).
- Calendar footer compacted: shared `compactButton` css helper (`padding: 8px 16px; font-size: 0.875rem; box-shadow: none`) for Discard/Done, footer container `$p` reduced from 2.5 (20px) to 1 (8px). Date input vertical padding bumped from `1.125 × spacing` to `1.25 × spacing` to match `StyledInput` height (used by email/text fields). TimeZone select chevron nudged 2px lower (`top: 5px` → `top: 7px` in `forms/Select.tsx`) for vertical centering.
